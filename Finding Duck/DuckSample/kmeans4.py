		...
		if(s > minduck):
			...
			for q in range(k):
				if(k == 1):
					...
					maskimg[int(d),int(b),:] = [254,254,254]
				else:
					b,d = center[q]
			...
			narr.append(([sx, sy]))
			nnarr.append(([int(d), int(b)]))
			
print("--- %s seconds ---" % (time.time() - start_time))
cv2.imwrite('KMean4.png', sampleimg)

print(nn)
nparr = np.array(narr)
nparr.tofile('narr4.dat')

nnparr = np.array(nnarr)
nnparr.tofile('nnarr4.dat')
