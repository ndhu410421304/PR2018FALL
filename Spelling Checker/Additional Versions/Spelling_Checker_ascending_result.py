...
class CheckResult():
	...
	def do_action(self):
		...
		self.D1 = sorted(self.D1, key = len)
		...
		if(self.equal == 1):
			...
			for i in range(0,self.D2C):
				R.append(self.D2[i] + " " + str(self.check_route(self.gen_route(self.D2[i]), I, len(self.D2[i]), 2)))
			R.append('Word with distance 3:')
			for i in range(0,self.D3C):
				R.append(self.D3[i] + " " + str(self.check_route(self.gen_route(self.D3[i]), I, len(self.D3[i]), 3)))
